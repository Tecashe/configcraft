...
//     id: "INITIALIZING",
//     title: "Initializing",
//     description: "Setting up generation process",
//     icon: Target,
//   },
//   {
//     id: "ANALYZING",
...
//     id: "COMPLETED",
//     title: "Completed",
//     description: "Your tool is ready!",
//     icon: CheckCircle,
//   },
// ]
...
//     return GENERATION_STEPS.find((step) => step.id === toolStatus.generationStatus)
//   }, [toolStatus])
...
//     async (toolId: string) => {
//       try {
//         console.log(`🔄 Polling status for tool: ${toolId}`)
//         const response = await fetch(`/api/organizations/${orgSlug}/tools/${toolId}/status`)
//         if (!response.ok) {
//           console.log(`❌ Status polling failed: ${response.status}`)
//           return
//         }

//         const status: ToolStatus = await response.json()
//         console.log(`📊 Status update for tool ${toolId}:`, {
//           status: status.status,
//           generationStatus: status.generationStatus,
//           progress: status.progress,
//           hasDemo: !!status.demoUrl,
//           hasError: !!status.error,
//           filesCount: status.files?.length || 0,
//         })
...
//         // Continue polling if still generating
//         if (status.status === "GENERATING") {
//           setTimeout(() => pollToolStatus(toolId), 2000) // Poll every 2 seconds
//         }
//       } catch (error) {
//         console.error("❌ Status polling error:", error)
//       }
...
//       const result = await response.json()
//       console.log("✅ Tool creation initiated:", result.toolId)
//       setCurrentToolId(result.toolId)
...
//     if (!toolStatus?.files || toolStatus.files.length === 0) return
...
//             {toolStatus && (
//               <div className="flex items-center space-x-2 px-3 py-1.5 bg-[#444444] rounded-full border border-[#444444]">
//                 <RefreshCw className={`h-4 w-4 text-[#B0B0B0] ${isGenerating ? "animate-spin" : ""}`} />
//                 <span className="text-sm font-medium text-[#E0E0E0]">
//                   {currentGenerationStep?.title || toolStatus.generationStatus}
//                 </span>
//               </div>
//             )}
//             <div className="flex items-center space-x-2 px-3 py-1.5 bg-[#444444] rounded-full border border-[#444444]">
...
//                       value="code"
//                       className="data-[state=active]:bg-[#888888] data-[state=active]:text-[#121212]"
//                       disabled={!toolStatus?.files || toolStatus.files.length === 0}
//                     >
//                       Code ({toolStatus?.files?.length || 0})
//                     </TabsTrigger>
//                   </TabsList>
...
//                           <div className="flex items-center space-x-2">
//                             <Code className="h-5 w-5 text-[#B0B0B0]" />
...
//                           {/* File List */}
//                           <div className="w-1/3 border-r border-[#444444] p-4">
//                             <h4 className="text-sm font-medium text-[#E0E0E0] mb-3">
//                               Files ({toolStatus.files.length})
//                             </h4>
...
//                                       ? "bg-[#888888] text-[#121212]"
//                                       : "bg-[#444444] text-[#E0E0E0] hover:bg-[#666666]"
//                                   }`}
...
//                                     variant="outline"
//                                     size="sm"
//                                     onClick={() => copyToClipboard(toolStatus.files[selectedFileIndex].content)}
//                                     className="border-[#444444] text-[#B0B0B0] hover:bg-[#444444] bg-transparent"
//                                   >
...
//                                   <pre className="text-xs text-[#E0E0E0] font-mono whitespace-pre-wrap">
//                                     <code>{toolStatus.files[selectedFileIndex].content}</code>
//                                   </pre>
//                                 </div>
//                               </>
//                             )}
...
//                             <Code className="h-12 w-12 text-[#B0B0B0] mx-auto mb-4" />
//                             <p className="text-[#B0B0B0]">No code files generated yet</p>
...
//                         <div className="flex items-center space-x-2">
//                           <Badge className="bg-[#444444] text-[#E0E0E0] border-0">{toolStatus.status}</Badge>
//                           {toolStatus.generationStatus && (
//                             <Badge variant="outline" className="border-[#444444] text-[#B0B0B0]">
//                               {toolStatus.generationStatus}
//                             </Badge>
//                           )}
//                         </div>
...
//                     <div className="space-y-4">
//                       {GENERATION_STEPS.map((step, index) => {
//                         const isActive = toolStatus?.generationStatus === step.id
//                         const isCompleted =
//                           toolStatus && GENERATION_STEPS.findIndex((s) => s.id === toolStatus.generationStatus) > index
//                         const isError = toolStatus?.status === "ERROR"
//                         const StepIcon = step.icon
...
//                     {toolStatus?.error && (
//                       <div className="space-y-3 pt-4 border-t border-[#444444]">
//                         <p className="text-sm font-medium text-red-400 flex items-center space-x-2">
//                           <AlertCircle className="h-4 w-4" />
//                           <span>Generation Error</span>
//                         </p>
//                         <div className="bg-red-500/10 border border-red-500/20 rounded-lg p-3">
//                           <p className="text-sm text-red-400">{toolStatus.error}</p>
//                         </div>
//                       </div>
//                     )}
//                   </CardContent>
...
//                       <div className="flex items-center space-x-3 mb-4">
//                         <Code className="h-5 w-5 text-[#B0B0B0]" />
//                         <span className="font-medium text-[#E0E0E0]">Debug Info</span>
//                       </div>
//                       <div className="space-y-2 text-xs text-[#B0B0B0]">
//                         <div>Tool ID: {toolStatus.id}</div>
//                         <div>Chat ID: {toolStatus.chatId || "Not set"}</div>
//                         <div>Files: {toolStatus.files?.length || 0}</div>
//                         <div>Created: {new Date(toolStatus.createdAt).toLocaleString()}</div>
//                         <div>Updated: {new Date(toolStatus.updatedAt).toLocaleString()}</div>
//                       </div>
//                     </CardContent>
//                   </Card>
//                 )}
...
//                       <li>• The AI learns from your feedback</li>
//                       <li>• Preview your tool before publishing</li>
//                       <li>• Check the Code tab to see generated files</li>
//                     </ul>
//                   </CardContent>
//                 </Card>
//               </div>
