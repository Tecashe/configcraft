...
export async function POST(request: NextRequest) {
  try {
    ...
    const latestVersion = (chatResponse as any).latestVersion
    const files = latestVersion ? mapSdkFilesToToolFiles(latestVersion.files) : []
    const demoUrl = latestVersion?.demoUrl || chatResponse.webUrl || null

    console.log("[v0-api] Files found in latestVersion:", files.length)
    console.log("[v0-api] demoUrl:", demoUrl)
    console.log("[v0-api] latestVersion.demoUrl:", latestVersion?.demoUrl)
    console.log("[v0-api] chatResponse.webUrl:", chatResponse.webUrl)
