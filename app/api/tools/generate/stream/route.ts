...
export async function POST(request: NextRequest) {
  try {
    const { toolName, requirements, category, userEmail, organizationId, selectedIntegrations } = await request.json()
...
async function generateToolWithRealStreaming(request: any, sendUpdate: (data: any) => void) {
  try {
    if (request.selectedIntegrations?.length > 0) {
      sendUpdate({
        type: "progress",
        step: "analyzing",
        progress: 5,
        message: "Verifying integration connections...",
        timestamp: Date.now(),
      })

      sendUpdate({
        type: "log",
        message: `🔗 Testing ${request.selectedIntegrations.length} selected integrations`,
        timestamp: Date.now(),
      })
    ...
    const result =
      request.organizationId && request.selectedIntegrations?.length > 0
        ? await v0ToolGenerator.generateToolWithRealData(request, request.organizationId, request.selectedIntegrations)
    ...
    const enhancedMetrics = {
      ...
      components: result.files.filter((f) => f.name.includes("component") || f.name.includes(".tsx")).length,
      apiEndpoints: result.files.filter((f) => f.name.includes("api") || f.name.includes("route")).length,
      estimatedValue: request.selectedIntegrations?.length > 0 ? "$5,000+" : "$2,500+",
      integrationsConfigured: request.selectedIntegrations?.length || 0,
    }
    ...
    sendUpdate({
      ...
      message:
        request.selectedIntegrations?.length > 0
          ? `Tool generated with ${request.selectedIntegrations.length} real data integrations!`
    ...
    sendUpdate({
      ...
      message:
        request.selectedIntegrations?.length > 0
          ? "🎉 Production-ready tool with real data integration completed!"
          : "🎉 Tool generation completed! Ready for preview and deployment.",
      timestamp: Date.now(),
    })
  } catch (error) {
    sendUpdate({
