...
//         files: [],
//         status: "error",
//         error: "V0 API key not configured. Please settit V0_API_KEY environment variable.",
//         progress: 0,
//         step: "error",
//       }
//     }
...
// const v0Service = new V0ToolGenerator()
// export const v0ToolGenerator = v0Service
...
//   async generateToolWithRealData(
//     request: ToolGenerationRequest,
//     organizationId: string,
//     selectedIntegrations: string[] = [],
//   ): Promise<ToolGenerationResult> {
//     const startTime = Date.now()
//     V0Logger.info("🚀 Starting tool generation with real integration data", {
//       toolName: request.toolName,
//       category: request.category,
//       organizationId,
...
//     try {
//       // Step 1: Get connected integrations and test them
//       const { integrationManager } = await import("@/lib/integration-manager")
//       const connectedIntegrations = await integrationManager.getConnectedIntegrations(organizationId)

//       // Filter to only selected integrations
//       const activeIntegrations = connectedIntegrations.filter((integration) =>
//         selectedIntegrations.includes(integration.provider.toLowerCase()),
//       )

//       V0Logger.info("🔗 Testing integration connections", {
//         totalConnected: connectedIntegrations.length,
//         selectedCount: activeIntegrations.length,
//         activeIntegrations: activeIntegrations.map((i) => i.name),
//       })

//       // Step 2: Test all connections before generation
//       const connectionTests = await integrationManager.testAllConnections(organizationId)

//       if (!connectionTests.allPassed && activeIntegrations.length > 0) {
//         V0Logger.warn("⚠️ Some integration connections failed", {
//           failures: connectionTests.failures,
//         })
//         // Continue with generation but note the failures
//       }

//       // Step 3: Fetch sample real data from integrations
//       const realDataSamples: Record<string, any[]> = {}

//       for (const integration of activeIntegrations) {
//         try {
//           const sampleData = await integrationManager.fetchRealData(integration, "default", 5)
//           realDataSamples[integration.provider] = sampleData
//           V0Logger.info(`📊 Fetched real data from ${integration.name}`, {
//             provider: integration.provider,
//             recordCount: sampleData.length,
//           })
//         } catch (error) {
//           V0Logger.warn(`⚠️ Could not fetch data from ${integration.name}`, { error })
//         }
//       }

//       // Step 4: Build enhanced prompt with real data context
//       const prompt = this.buildToolPromptWithRealData(
//         request.toolName,
...
//       V0Logger.info("✅ Enhanced prompt built with real data context", {
//         promptLength: prompt.length,
//         integrationsIncluded: activeIntegrations.length,
//         realDataSources: Object.keys(realDataSamples).length,
//       })

//       // Step 5: Generate with v0 SDK
//       const chat: any = await v0.chats.create({
//         message: prompt,
//         system: `You are an expert React and TypeScript developer who creates production-ready business applications with REAL data integrations. 

// CRITICAL REQUIREMENTS:
// - Use the provided real data samples to understand actual data structures
// - Generate TypeScript interfaces that match the real data schemas
// - Include proper error handling and loading states for all API calls
// - Add data transformation utilities for API response mapping
// - Implement proper authentication for each integration
// - Use environment variables for API keys and configuration
// - Generate complete, production-ready code with real data integration
// - Include comprehensive error boundaries and fallback UI

// The generated application must work with REAL data from the connected integrations, not mock data.`,
//         chatPrivacy: "private",
//         modelConfiguration: {
//           modelId: "v0-1.5-md",
//           imageGenerations: false,
...
//       const chatEndTime = Date.now()
//       V0Logger.info(`✅ Chat created with real data integration`, {
//         chatId: chat.id,
//         chatUrl: chat.url,
//         demoUrl: chat.demo,
//         filesCount: chat.files?.length || 0,
//         creationTime: `${chatEndTime - startTime}ms`,
...
//       // Step 7: Inject environment variables and integration setup
//       const enhancedFiles = this.injectIntegrationSetup(files, activeIntegrations)
...
//       const totalTime = Date.now() - startTime
//       V0Logger.info("🎉 Tool generation with real data completed", {
//         ...result,
//         totalGenerationTime: `${totalTime}ms`,
//         integrationsConfigured: activeIntegrations.length,
//         realDataSources: Object.keys(realDataSamples).length,
//       })
...
//     } catch (error) {
//       const totalTime = Date.now() - startTime
//       V0Logger.error("💥 Real data generation error", {
//         error:
//           error instanceof Error
//             ? {
//                 name: error.name,
...
//         files: [],
//         status: "error",
//         error: error instanceof Error ? error.message : "Real data generation failed",
//         progress: 0,
//         step: "error",
//       }
//     }
...
//   private buildToolPromptWithRealData(
//     toolName: string,
//     requirements: string,
//     category: string,
//     integrations: any[],
//     realDataSamples: Record<string, any[]>,
//   ): string {
//     const basePrompt = this.buildToolPrompt(toolName, requirements, category)
...
// Real Data Samples:
// ${Object.entries(realDataSamples)
//   .map(
//     ([provider, data]) => `
// ${provider.toUpperCase()} Sample Data:
...
// 1. Create TypeScript interfaces that match the EXACT structure of the real data samples above
// 2. Generate API utility functions for each connected integration
// 3. Use proper environment variables for API keys:
//    ${integrations.map((i) => `- ${i.provider.toUpperCase()}_API_KEY`).join("\n   ")}
// 4. Include proper error handling for each integration
// 5. Add loading states for all data fetching operations
// 6. Implement data transformation utilities to map API responses to UI components
// 7. Generate realistic UI components that display the actual data structure
// 8. Include proper authentication and API client setup for each integration
// 9. Add data refresh capabilities and real-time updates where applicable
// 10. Ensure all generated code works with PRODUCTION data, not mock data

// The application must be immediately deployable and functional with the user's real business data.`

//     return basePrompt + integrationContext
//   }

//   private injectIntegrationSetup(files: ToolFile[], integrations: any[]): ToolFile[] {
//     if (integrations.length === 0) {
//       return files
//     }

//     // Add environment variables file
//     const envFile: ToolFile = {
//       name: ".env.example",
//       content: `# Integration API Keys
// ${integrations
//   .map((integration) => `${integration.provider.toUpperCase()}_API_KEY=your_${integration.provider}_api_key_here`)
//   .join("\n")}
...
//     // Add integration utilities file
//     const integrationUtilsFile: ToolFile = {
//       name: "lib/integrations.ts",
//       content: `// Integration utilities for connected services
// ${integrations
//   .map((integration) => {
...
//     const response = await fetch('/api/stripe/customers?limit=' + limit)
//     return response.json()
//   },
//   async getPayments(limit = 10) {
//     const response = await fetch('/api/stripe/payments?limit=' + limit)
//     return response.json()
//   }
// }`
//       case "supabase":
...
// export const ${integration.provider}Client = {
//   // Add your ${integration.name} API calls here
// }`
//     }
//   })
//   .join("\n")}
...
//   ${integrations.map((i) => `${i.provider}: true`).join(",\n  ")}
// }
// `,
//       type: "typescript",
//     }
...
// const v0ToolGenerator = new V0ToolGenerator()

// export default v0ToolGenerator
// export { v0ToolGenerator }
...
export class V0ToolGenerator {
  ...
  constructor() {
    // Initialize v0 client with proper API key configuration
    this.initializeV0Client()
  }

  private initializeV0Client(): void {
    const apiKey = process.env.V0_API_KEY
    
    if (!apiKey) {
      V0Logger.error("❌ V0_API_KEY environment variable not found")
      V0Logger.error("💡 Please obtain your API key from https://v0.dev/chat/settings/keys")
      V0Logger.error("💡 Then set it as: V0_API_KEY=your_api_key_here")
    ...
    try {
      ...
      V0Logger.info("✅ V0 client initialized successfully", {
        keyLength: apiKey.length,
        keyPrefix: apiKey.substring(0, 8) + "...",
      })
    } catch (error) {
      V0Logger.error("❌ Failed to initialize v0 client", { error })
      throw new Error("Failed to initialize v0 client")
    }
  }
  ...
  private validateApiKey(): boolean {
    ...
    if (!apiKey) {
      V0Logger.error("❌ V0_API_KEY environment variable not found")
      V0Logger.error("💡 To get your API key:")
      V0Logger.error("   1. Go to https://v0.dev/chat/settings/keys")
      V0Logger.error("   2. Create a new API key")
      V0Logger.error("   3. Set it as environment variable: V0_API_KEY=your_key_here")
      return false
    }

    if (apiKey.length < 10) {
      V0Logger.error("❌ V0_API_KEY appears to be invalid (too short)")
      return false
    }

    V0Logger.info("✅ V0 API key validated", {
      keyLength: apiKey.length,
      keyPrefix: apiKey.substring(0, 8) + "...",
    })
    return true
  ...
  async generateTool(request: ToolGenerationRequest): Promise<ToolGenerationResult> {
    ...
    if (!this.validateApiKey()) {
      return {
        ...
        files: [],
        status: "error",
        error: "V0 API key not configured. Please set V0_API_KEY environment variable. Get your key from https://v0.dev/chat/settings/keys",
        progress: 0,
        step: "error",
      }
    }
    ...
    try {
      ...
      // Step 2: Create chat with v0 SDK using proper configuration
      V0Logger.info("🤖 Creating chat with v0 SDK", {
        modelId: "v0-1.5-md",
        chatPrivacy: "private",
        imageGenerations: false,
      ...
      // Use the initialized v0 client
      const chat: any = await this.v0Client.chats.create({
        message: prompt,
        system: "You are an expert React and TypeScript developer who creates professional business applications with modern UI/UX patterns. Always generate complete, production-ready code with proper error handling, TypeScript types, and responsive design.",
        chatPrivacy: "private",
        modelConfiguration: {
          modelId: "v0-1.5-md",
          imageGenerations: false,
      ...
      if (error instanceof Error) {
        ...
        if (error.message.includes("401") || error.message.includes("Unauthorized") || error.message.includes("API key")) {
          errorMessage = "Invalid V0 API key. Please check your V0_API_KEY environment variable. Get your key from https://v0.dev/chat/settings/keys"
        } else if (error.message.includes("429") || error.message.includes("rate limit")) {
          errorMessage = "Rate limit exceeded. Please try again in a few minutes."
        } else if (error.message.includes("400") || error.message.includes("Bad Request")) {
          errorMessage = "Invalid request parameters. Please check your tool requirements."
          ...
          errorMessage = "Network error connecting to v0 API. Please check your internet connection."
        }
      }
  ...
  async regenerateTool(chatId: string, feedback: string): Promise<ToolGenerationResult> {
    ...
    if (!this.validateApiKey()) {
      return {
        ...
        files: [],
        status: "error",
        error: "V0 API key not configured. Get your key from https://v0.dev/chat/settings/keys",
        progress: 0,
        step: "error",
      }
    }
    ...
    try {
      ...
      const messageStartTime = Date.now()
      await this.v0Client.chats.sendMessage({
        chatId,
        message: `Please improve the tool based on this feedback: ${feedback}
...
Make sure to maintain all the original requirements while incorporating these improvements. Keep the code production-ready with proper TypeScript types, error handling, and responsive design.`,
      ...
      const fetchStartTime = Date.now()
      const updatedChat: any = await this.v0Client.chats.getById({ chatId })
      const fetchEndTime = Date.now()
      ...
      let errorMessage = error instanceof Error ? error.message : "Regeneration failed"
  ...
  async getChatHistory(chatId: string) {
    ...
    try {
      const chatStartTime = Date.now()
      const chat: any = await this.v0Client.chats.getById({ chatId })
      const chatEndTime = Date.now()
      ...
      const messagesStartTime = Date.now()
      const messagesResponse = await this.v0Client.chats.findMessages({ chatId })
      const messagesEndTime = Date.now()
  ...
  async continueChat(chatId: string, message: string): Promise<ToolGenerationResult> {
    ...
    try {
      const sendStartTime = Date.now()
      await this.v0Client.chats.sendMessage({ chatId, message })
      const sendEndTime = Date.now()
      ...
      let errorMessage = error instanceof Error ? error.message : "Chat continuation failed"
      if (errorMessage.includes("401") || errorMessage.includes("API key")) {
        errorMessage = "Invalid V0 API key. Please check your V0_API_KEY environment variable."
      }
  ...
  async listChats(limit = 20, offset = 0) {
    ...
    try {
      const startTime = Date.now()
      const chatsResponse = await this.v0Client.chats.find({
        limit: String(limit),
        offset: String(offset),
      })
      const endTime = Date.now()
  ...
  async getGenerationStatus(chatId: string): Promise<ToolGenerationResult> {
    ...
    try {
      ...
      let errorMessage = error instanceof Error ? error.message : "Status check failed"
      if (errorMessage.includes("401") || errorMessage.includes("API key")) {
        errorMessage = "Invalid V0 API key. Please check your V0_API_KEY environment variable."
      }
...
The generated application must work with REAL data from the connected integrations, not mock data.`,
      ...
      let errorMessage = error instanceof Error ? error.message : "Real data generation failed"
      if (errorMessage.includes("401") || errorMessage.includes("API key")) {
        errorMessage = "Invalid V0 API key. Please check your V0_API_KEY environment variable."
      }
